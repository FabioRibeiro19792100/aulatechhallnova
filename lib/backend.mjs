...
export async function saveRemoteAppState(payload) {
  ...
  // Cada tentativa: lê updated_at → merge → PATCH condicional (WHERE updated_at = <lido>).
  // Supabase retorna a linha atualizada com return=representation: array vazio = colisão → retry.
  ...
  for (let attempt = 0; attempt < MAX_RETRIES; attempt++) {
    ...
    // 2. Merge remoto + incoming
    const merged = mergeEventCollections(currentEvents, payload.events);
    const updatedAt = new Date().toISOString();

    // 3a. Sem registro ainda — upsert simples
    if (!currentVersion) {
    ...
    // 3b. PATCH condicional: só atualiza se updated_at ainda é o que lemos
    // return=representation devolve a linha se atualizou, array vazio se colisão
    const updated = await fetchSupabase(
      `/rest/v1/app_state?id=eq.${remoteStateKey}&updated_at=eq.${encodeURIComponent(currentVersion)}`,
      {
        method: "PATCH",
    ...
    // Colisão — outra instância escreveu antes. Backoff com jitter e retry.
