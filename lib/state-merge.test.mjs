...
test("mergeAppEvents does not mutate its inputs", () => {
  const remote = [E({ stressEntries: [{ id: "a" }] })];
  const incoming = [E({ stressEntries: [{ id: "b" }] })];
  const remoteSnapshot = JSON.stringify(remote);
  const incomingSnapshot = JSON.stringify(incoming);
  mergeAppEvents(remote, incoming);
  assert.equal(JSON.stringify(remote), remoteSnapshot);
  assert.equal(JSON.stringify(incoming), incomingSnapshot);
});

test("mixed identified/id-less array does not duplicate orphans across re-merges", () => {
  const remote = [E({ items: [{ id: "k1" }, { note: "loose" }] })];
  const incoming = [E({ items: [{ id: "k1" }, { note: "loose" }] })];
  const first = mergeAppEvents(remote, incoming);
  const second = mergeAppEvents(first, first);
  const loose = second[0].items.filter((x) => x.note === "loose");
