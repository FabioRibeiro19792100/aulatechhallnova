...
export async function saveRemoteAppState(payload) {
  ...
  return withWriteLock(async () => {
    ...
    if (!process.env.SUPABASE_URL || !process.env.SUPABASE_ANON_KEY) {
      // Local: lê → merge → escreve dentro do lock
    ...
    // Supabase: lê → merge → escreve dentro do lock
    const rows = await fetchSupabase(
      `/rest/v1/app_state?id=eq.${remoteStateKey}&select=payload&limit=1`,
    );
    const currentEvents = Array.isArray(rows?.[0]?.payload?.events)
      ? rows[0].payload.events
      : [];
    const merged = mergeEventCollections(currentEvents, payload.events);
