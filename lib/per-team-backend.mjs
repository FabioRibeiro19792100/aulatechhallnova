...
async function realFetch(pathname, { method = "GET", body, headers = {}, timeoutMs = 20000 } = {}) {
...
export async function putEventStateOCC(eventId, { payload, expected_version }) {
  ...
  const row = current?.[0];
  if (!row) {
    if (expected_version !== 0) throw notFound(`event_state ${eventId} nao encontrado.`);
    const inserted = await db("/rest/v1/event_state", {
      method: "POST",
      headers: { Prefer: "resolution=ignore-duplicates,return=representation" },
      body: [{ event_id: eventId, payload, version: 1, updated_at: updatedAt }],
    });
    ...
    const reread = await db(`/rest/v1/event_state?event_id=eq.${encodeURIComponent(eventId)}&select=payload,version,updated_at&limit=1`);
    const rereadRow = reread?.[0];
    if (!rereadRow) throw notFound(`event_state ${eventId} nao encontrado.`);
    throw conflict({ current_payload: rereadRow.payload, current_version: rereadRow.version, current_updated_at: rereadRow.updated_at });
  }
...
export async function deleteTeamScopedData(eventId, teamIdx) {
  const ev = encodeURIComponent(eventId);
  await Promise.all([
    db(`/rest/v1/team_state?event_id=eq.${ev}&team_idx=eq.${teamIdx}`, { method: "DELETE", headers: { Prefer: "return=minimal" } }),
    db(`/rest/v1/team_presence?event_id=eq.${ev}&team_idx=eq.${teamIdx}`, { method: "DELETE", headers: { Prefer: "return=minimal" } }),
    db(`/rest/v1/executions?event_id=eq.${ev}&team_idx=eq.${teamIdx}`, { method: "DELETE", headers: { Prefer: "return=minimal" } }),
    db(`/rest/v1/token_operational_logs?event_id=eq.${ev}&team_idx=eq.${teamIdx}`, { method: "DELETE", headers: { Prefer: "return=minimal" } }),
    db(`/rest/v1/help_requests?event_id=eq.${ev}&team_idx=eq.${teamIdx}`, { method: "DELETE", headers: { Prefer: "return=minimal" } }),
  ]);
  return { ok: true };
}

export async function deleteAllEventData(eventId) {
  const ev = encodeURIComponent(eventId);
  await Promise.all([
    db(`/rest/v1/team_state?event_id=eq.${ev}`, { method: "DELETE", headers: { Prefer: "return=minimal" } }),
    db(`/rest/v1/team_presence?event_id=eq.${ev}`, { method: "DELETE", headers: { Prefer: "return=minimal" } }),
    db(`/rest/v1/executions?event_id=eq.${ev}`, { method: "DELETE", headers: { Prefer: "return=minimal" } }),
    db(`/rest/v1/token_operational_logs?event_id=eq.${ev}`, { method: "DELETE", headers: { Prefer: "return=minimal" } }),
    db(`/rest/v1/help_requests?event_id=eq.${ev}`, { method: "DELETE", headers: { Prefer: "return=minimal" } }),
  ]);
  await db(`/rest/v1/event_state?event_id=eq.${ev}`, { method: "DELETE", headers: { Prefer: "return=minimal" } });
  return { ok: true };
}

export async function deleteTeamExecutions(eventId, teamIdx, missionId) {
  const ev = encodeURIComponent(eventId);
  const parts = [`event_id=eq.${ev}`, `team_idx=eq.${teamIdx}`];
  if (missionId) parts.push(`mission_id=eq.${encodeURIComponent(missionId)}`);
  await db(`/rest/v1/executions?${parts.join("&")}`, { method: "DELETE", headers: { Prefer: "return=minimal" } });
  return { ok: true };
}

export async function patchExecutionPayload(id, payloadPatch) {
  const rows = await db(`/rest/v1/executions?id=eq.${encodeURIComponent(id)}&select=payload&limit=1`);
  const row = rows?.[0];
  if (!row) throw notFound(`execution ${id} nao encontrado.`);
  const merged = { ...(row.payload || {}), ...(payloadPatch || {}) };
  await db(`/rest/v1/executions?id=eq.${encodeURIComponent(id)}`, {
    method: "PATCH",
    headers: { Prefer: "return=minimal" },
    body: { payload: merged },
  });
...
export async function listExecutions({ event_id, team_idx, mission_id, limit = 50, timeoutMs } = {}) {
  ...
  const rows = await db(`/rest/v1/executions?${parts.join("&")}`, timeoutMs ? { timeoutMs } : undefined);
  return rows || [];
}

export async function listExecutionSummaries({ event_id, limit = 6000, timeoutMs } = {}) {
  const parts = [
    `event_id=eq.${encodeURIComponent(event_id)}`,
    "select=id,team_idx,mission_id,kind,created_at,tokens,custo",
...
export async function listTokenLogs({ event_id, team_idx, limit = 200, timeoutMs } = {}) {
  const parts = [`event_id=eq.${encodeURIComponent(event_id)}`];
  if (team_idx !== undefined && team_idx !== null) parts.push(`team_idx=eq.${team_idx}`);
  parts.push(`order=created_at.desc`);
  parts.push(`limit=${limit}`);
  const rows = await db(`/rest/v1/token_operational_logs?${parts.join("&")}`, timeoutMs ? { timeoutMs } : undefined);
  return rows || [];
}
...
export async function listHelpRequests({ event_id, status, limit = 100, timeoutMs } = {}) {
  const parts = [`event_id=eq.${encodeURIComponent(event_id)}`];
  if (status) parts.push(`status=eq.${encodeURIComponent(status)}`);
  parts.push(`order=created_at.desc`);
  parts.push(`limit=${limit}`);
  const rows = await db(`/rest/v1/help_requests?${parts.join("&")}`, timeoutMs ? { timeoutMs } : undefined);
  return rows || [];
}
...
export async function listPresence(event_id, { timeoutMs } = {}) {
  const rows = await db(
    `/rest/v1/team_presence?event_id=eq.${encodeURIComponent(event_id)}&select=event_id,team_idx,member_name,last_seen_at`,
...
export async function getDashboard(eventId) {
  const [event, teams, presence, helpRequests, executionSummaries, tokenLogs] = await Promise.all([
    getEventState(eventId).catch((e) => (e.statusCode === 404 ? null : Promise.reject(e))),
    db(`/rest/v1/team_state?event_id=eq.${encodeURIComponent(eventId)}&select=team_idx,payload,version,updated_at`, { timeoutMs: 20000 }),
    listPresence(eventId, { timeoutMs: 20000 }),
    listHelpRequests({ event_id: eventId, limit: 100, timeoutMs: 20000 }),
    listExecutionSummaries({ event_id: eventId, limit: 6000, timeoutMs: 20000 }),
    listTokenLogs({ event_id: eventId, limit: 100, timeoutMs: 20000 }),
  ]);
  const rows = executionSummaries || [];
  return {
    event,
    teams: teams || [],
    presence,
    helpRequests,
    recentExecutions: rows.filter((row) => row.mission_id !== "__training__"),
    trainingExecutions: rows.filter((row) => row.mission_id === "__training__"),
    tokenLogs,
    serverNow: nowIso(),
  };
}
