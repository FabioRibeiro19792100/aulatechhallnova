...
const localBundledPythonPath = path.join(
  os.homedir(),
  ".cache/codex-runtimes/codex-primary-runtime/dependencies/python/bin/python3",
);
const extractionScriptPath = path.resolve("scripts/extract_attachment.py");
...
function resolvePythonBinary() {
  ...
  if (existsSync(localBundledPythonPath)) return localBundledPythonPath;
  return "python3";
}
...
export async function extractDocumentText({ fileName, contentBase64 }) {
  ...
  return new Promise((resolve, reject) => {
    const child = spawn(pythonBin, [extractionScriptPath], {
    ...
    child.on("error", (error) => {
      const wrapped = new Error(`Falha ao iniciar extrator de arquivos: ${error.message}`);
    ...
    child.on("close", (code) => {
      if (code !== 0) {
        const error = new Error(stderr.trim() || "Falha ao extrair texto do arquivo.");
        error.statusCode = 500;
        reject(error);
        return;
      }
      ...
      try {
        ...
        const error = new Error("Extrator retornou resposta invalida.");
        error.statusCode = 500;
        reject(error);
      }
    });

    child.stdin.write(JSON.stringify({ fileName, contentBase64 }));
    child.stdin.end();
  });
}
