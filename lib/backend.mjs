...
import { mergeAppEvents } from "./state-merge.mjs";
...
export async function saveRemoteAppState(payload) {
  ...
  for (let attempt = 0; attempt < MAX_RETRIES; attempt++) {
    ...
    const updatedAt = new Date().toISOString();
    const merged = mergeAppEvents(currentEvents, payload.events);

    if (JSON.stringify(merged) === JSON.stringify(currentEvents)) {
      readCache = { events: merged, updatedAt: currentUpdatedAt ?? updatedAt };
      readCacheExpiresAt = Date.now() + READ_CACHE_TTL_MS;
      return { ok: true, updatedAt: currentUpdatedAt ?? updatedAt, serverNow: updatedAt };
    }
