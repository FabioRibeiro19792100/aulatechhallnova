...
export async function getRemoteAppState() {
  ...
  if (!process.env.SUPABASE_URL || !process.env.SUPABASE_ANON_KEY) {
    ...
    return {
      events: Array.isArray(localState?.events) ? localState.events : [],
...
export async function saveRemoteAppState(payload) {
  ...
  // Local: write lock em memória é suficiente (processo único)
  if (!process.env.SUPABASE_URL || !process.env.SUPABASE_ANON_KEY) {
    return withWriteLock(async () => {
      const updatedAt = new Date().toISOString();
      const current = await readLocalAppState();
  ...
  // Supabase (serverless): optimistic locking com retry.
  // Lê o updated_at atual, escreve só se não mudou (WHERE updated_at = <lido>).
  // Se outra instância escreveu antes, relê, faz merge e tenta de novo.
  const MAX_RETRIES = 6;
  const BASE_DELAY_MS = 60;

  for (let attempt = 0; attempt < MAX_RETRIES; attempt++) {
    // 1. Lê estado atual + versão
    const rows = await fetchSupabase(
      `/rest/v1/app_state?id=eq.${remoteStateKey}&select=payload,updated_at&limit=1`,
    );
    const current = rows?.[0];
    const currentEvents = Array.isArray(current?.payload?.events) ? current.payload.events : [];
    ...
    // 3. Escreve somente se updated_at não mudou (conditional update)
    const filterVersion = currentVersion
      ? `&updated_at=eq.${encodeURIComponent(currentVersion)}`
      : "";
    const result = await fetchSupabase(
      `/rest/v1/app_state?id=eq.${remoteStateKey}${filterVersion}`,
      {
        method: "PATCH",
    ...
    // PATCH retorna null/vazio quando atualiza 0 linhas (versão mudou — retry)
    // e null/vazio quando atualiza com return=minimal (sucesso)
    // Diferenciamos pelos headers: Supabase retorna Content-Range com rowcount
    // Usamos um upsert na primeira tentativa se não há registro ainda
    ...
    // Verifica se o PATCH atualizou algo relendo o updated_at
    const verify = await fetchSupabase(
      `/rest/v1/app_state?id=eq.${remoteStateKey}&select=updated_at&limit=1`,
    );
    if (verify?.[0]?.updated_at === updatedAt) {
      return { ok: true, updatedAt, serverNow: updatedAt };
    }

    // Outra instância ganhou — espera com jitter e tenta de novo
    const delay = BASE_DELAY_MS * (attempt + 1) + Math.random() * BASE_DELAY_MS;
    await new Promise((resolve) => setTimeout(resolve, delay));
  }

  const error = new Error("Falha ao salvar estado: muitas tentativas concorrentes.");
